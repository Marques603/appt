<x-app-layout>
    <!-- Título da Página -->
    <x-page-title page="Lista de Categoria" pageUrl="{{ route('categorie.index') }}" header="{{ isset($categorie) ? 'Editar Categoria' : 'Criar Categoria' }}" />
        ...
        <section class="col-span-1 flex w-full flex-1 flex-col gap-6 lg:col-span-3 lg:w-auto">
            <div class="card">
                <div class="card-body">
                    ...
                    <p class="mb-4 text-sm font-normal text-slate-400">Preencha as informações da categoria</p>

                    <form method="POST" action="{{ isset($categorie) ? route('categorie.update', $categorie) : route('categorie.store') }}" class="flex flex-col gap-6">
                        @csrf
                        ...
                        <div class="flex items-center justify-end gap-4">
                            ...
                            <button type="submit" class="btn btn-primary">
                                {{ isset($categorie) ? 'Salvar' : 'Adicionar' }}
                            </button>
                        </div>
                    </form>
                </div>

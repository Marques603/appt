<x-app-layout>
    <x-page-title page="Tipos de Convênio" header="Editar Tipo de Convênio" />
    @section('title', 'Editar Tipo de Convênio | Inusittá')
        ...
        <section class="col-span-1 flex w-full flex-1 flex-col gap-6 lg:col-span-3 lg:w-auto">
            <div class="card">
                <div class="card-body">
                    ...
                    <form action="{{ route('agreements_type.update', $type->id) }}" method="POST" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <section class="rounded-lg bg-white p-6 shadow-sm dark:bg-slate-800 space-y-4">
                            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                ...
                                <div class="flex flex-col">
                                    ...
                                    <input type="text" name="type" id="type" class="input" required placeholder="Tipo do convênio" value="{{ old('type', $type->type) }}">
                                </div>

                                {{-- Descrição --}}
                                <div class="flex flex-col sm:col-span-2">
                                    <label for="description" class="label label-required font-medium">Descrição</label>
                                    <textarea name="description" id="description" rows="4" class="input" required placeholder="Descreva o tipo do convênio">{{ old('description', $type->description) }}</textarea>
                                </div>
                            </div>
                        </section>
